...
class Products extends Model { 
	...
	public function getAvailableOptions($filters = array()) {
		...
		$options = $this->getAvaibleValuesFromOptions($groups, $ids);

		return $options;
	}

	public function getAvaibleValuesFromOptions($groups, $ids) {
		$array = array();
		$options = new Options();
	...
	public function getList($offset = 0, $limit = 3, $filters = array(), $random = false) {
		...
		$sql = "SELECT 
			*,
			...
			as brand_name,
			(select categories.name from categories where categories.id = products.
			id_category ) as category_name
		FROM
			products 
		WHERE ".implode(' AND ', $where)."
		".$orderBySQL."
